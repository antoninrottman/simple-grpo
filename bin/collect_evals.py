...
def find_run_metadata(root: Path) -> Iterable[Path]:
    """Yield all run_metadata.json files from one or more result trees.

    You can override the search with env var RESULTS_GLOBS, a comma-separated
    list of glob patterns relative to the repo root. Example:
      RESULTS_GLOBS="outputs/results_run_*/**/run_metadata.json,outputs/*_sweep_*/*/run_metadata.json"

    Defaults cover common layouts: date-stamped runs and *_sweep_* folders.
    """

    patterns_env = os.getenv("RESULTS_GLOBS")
    if patterns_env:
        patterns = [p.strip() for p in patterns_env.split(",") if p.strip()]
    else:
        patterns = [
            # Generic date-stamped result runs
            "outputs/results_run_*/**/run_metadata.json",
            # Generic sweep folders (e.g., llama_r_sweep_results, qwen-3B_sweep_26sept, etc.)
            "outputs/*_sweep_*/*/run_metadata.json",
            "outputs/*_sweep_*/*/**/run_metadata.json",
        ]
